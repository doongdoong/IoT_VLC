...
void loop() {
  ...
  if (new_word == 1) {
    ...
    if ((byte_added = add_byte_to_frame(frame_buffer, &frame_index, &frame_size, &frame_state, received_data)) > 0) {
      ...
      if (id.equals("GV")) { //id가 에어컨일 때;
        if (command.equals("On")) { //명령어가 off이면
          for (int i = 544; i < 1472; i++) {
            digitalWrite(servoPin, HIGH);
            delayMicroseconds(i);
            digitalWrite(servoPin, LOW);
            delayMicroseconds(m_max - i);
          }
        } else if (command.equals("Off")) { //명령어가 On일때
          for (int i = 1472; i > 544; i--) {
            digitalWrite(servoPin, HIGH);
            delayMicroseconds(i);
            digitalWrite(servoPin, LOW);
            delayMicroseconds(m_max - i);
